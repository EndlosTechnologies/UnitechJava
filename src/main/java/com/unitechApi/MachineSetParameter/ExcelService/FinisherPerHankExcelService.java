...
public class FinisherPerHankExcelService {
    private XSSFWorkbook xssfWorkbook;
    private XSSFSheet xssfSheet;
    private List<FinisherperHank> finisherperHankData;

    public FinisherPerHankExcelService(List<FinisherperHank> finisherperHankData) {
        this.finisherperHankData = finisherperHankData;
        xssfWorkbook=new XSSFWorkbook();
    }
    private void writeHeaderLine() {
        xssfSheet=xssfWorkbook.createSheet("finisher Data");
    ...
    private void writeDataLine() {
       ...
       // xssfSheet.addMergedRegion(CellRangeAddress.valueOf("G3:H3"));
        ...
        // styleHeader.set
        createCell(rowHeader2,1,"Finisher In Hank ",styleHeader);
        ...
        style.setAlignment(HorizontalAlignment.CENTER);
        style.setFont(font);
        for (FinisherperHank finisher :finisherperHankData){
            Row row=xssfSheet.createRow(rowcount++);
            int countRow=1;
            createCell(row,countRow++,finisher.getFinisherhankMachine().getName(),style);
            createCell(row,countRow++,finisher.getDeliveryspeed(),style);
            createCell(row,countRow++,finisher.getMachineefficency(),style);
            createCell(row,countRow++,finisher.getProductiononratekgdfper8hour(),style);
            createCell(row,countRow++,finisher.getMachineefficencykgdfper6hours(),style);
            createCell(row,countRow++,finisher.getMachineefficencykgdfperday(),style);
            createCell(row,countRow++,finisher.getShift_a_sixHoursOne(),style);
            createCell(row,countRow++,finisher.getAvervg_difference_a_sixHoursOne(),style);
            createCell(row,countRow++,finisher.getShift_a_sixHoursTwo(),style);
            createCell(row,countRow++,finisher.getAvervg_difference_a_sixHoursTwo(),style);
            createCell(row,countRow++,finisher.getTotal_shift_prod_a(),style);
            createCell(row,countRow++,finisher.getShift_b_sixHoursOne(),style);
            createCell(row,countRow++,finisher.getAvervg_difference_b_sixHoursOne(),style);
            createCell(row,countRow++,finisher.getShift_b_sixHoursTwo(),style);
            createCell(row,countRow++,finisher.getAvervg_difference_b_sixHoursTwo(),style);
            createCell(row,countRow++,finisher.getTotal_shift_prod_b(),style);
            createCell(row,countRow++,finisher.getActual_producation(),style);
            createCell(row,countRow++,finisher.getEfficiency(),style);
            createCell(row,countRow++,finisher.getTarget_prod_variance_kg(),style);
            createCell(row,countRow++,finisher.getTarget_prod_variance(),style);
